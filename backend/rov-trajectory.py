...
def mavlink_worker():
    ...
    while True:
        master = None
        try:
            print(f"[MAVLINK] Mendengarkan paket UDP di {MAVLINK_UDP_ENDPOINT} ...")
            ...
            while True:
                ...
                if hb is not None:
                    ...
                    if src_sys != 0:
                        ...
                        print(f"[MAVLINK] Heartbeat terdeteksi -> Sys ID: {target_sys}, Comp ID: {target_comp}")
                        break

                if time.time() - t_start > HEARTBEAT_TIMEOUT_S:
                    raise Exception("Timeout menunggu heartbeat valid dari Pixhawk")
                ...
                if msg:
                    ...
                    with state_lock:
                        ...
                        # --- C. ESTIMASI POSISI X & Y (DEAD RECKONING) ---
                            ...
                            v_surge = dev_surge * K_SURGE  # Kecepatan linier maju/mundur (m/s)
                            ...
                            #    - Nilai == 1500: Netral/Diam (0)
                            dev_sway = dev_s5
                            v_sway = dev_sway * K_SWAY  # Kecepatan linier geser kanan/kiri (m/s)
                            ...
                            dx = (v_surge * math.sin(yaw_rad) + v_sway * math.cos(yaw_rad)) * dt
                            dy = (v_surge * math.cos(yaw_rad) - v_sway * math.sin(yaw_rad)) * dt
                            ...
                            if now_log - last_log_time >= 0.5:
                                ...
                                surge_lbl = "MAJU" if dev_surge > 0 else ("MUNDUR" if dev_surge < 0 else "DIAM")
                                sway_lbl = "KANAN" if dev_sway > 0 else ("KIRI" if dev_sway < 0 else "DIAM")
                                print(
...
if __name__ == '__main__':
    ...
    print(f"[ROV TRAJECTORY] Server Backend aktif pada http://127.0.0.1:{HTTP_PORT}")
